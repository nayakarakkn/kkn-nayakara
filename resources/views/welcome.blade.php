<x-layout>
    ...
    <!-- 7. Layanan Pengaduan & Aspirasi Warga Section -->
    <div id="pengaduan" class="py-20 bg-slate-50 relative border-t border-b border-slate-200/80">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="text-center mb-10">
                ...
                <h2 class="text-3xl font-bold text-slate-900 sm:text-4xl mb-3">Layanan Pengaduan <span class="text-emerald-600">Keranggan</span></h2>
                <p class="text-slate-500 text-sm max-w-lg mx-auto">Sampaikan saran, laporan perbaikan fasilitas, atau aspirasi Anda. Pengurus Keranggan siap menindaklanjuti demi kenyamanan lingkungan bersama.</p>
            </div>

            @if(session('success_complaint'))
            <div class="mb-8 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 flex items-start gap-3 shadow-sm">
                <svg class="w-6 h-6 text-emerald-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <div>
                    <h4 class="font-bold text-sm">Laporan Berhasil Diterima!</h4>
                    <p class="text-xs text-emerald-700 mt-0.5">{{ session('success_complaint') }}</p>
                </div>
            </div>
            @endif

            <div class="bg-white text-slate-900 rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-200/80">
                <form action="{{ route('complaint.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Nama Lengkap Warga <span class="text-red-500">*</span></label>
                            <input type="text" name="name" required placeholder="Contoh: Bapak Ahmad Syarif" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:bg-white transition-colors">
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">RT / Alamat Rumah</label>
                            <input type="text" name="rt_rw" placeholder="Contoh: Keranggan RT 03 / RW 01" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:bg-white transition-colors">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Kategori Laporan <span class="text-red-500">*</span></label>
                            <select name="category" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:bg-white transition-colors">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1">Isi Laporan / Aspirasi <span class="text-red-500">*</span></label>
                        <textarea name="content" rows="4" required placeholder="Jelaskan detail laporan atau aspirasi Anda secara singkat dan jelas..." class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 focus:bg-white transition-colors"></textarea>
                    </div>

                    <div class="pt-2 flex justify-end">
                        <button type="submit" class="w-full sm:w-auto px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm rounded-xl shadow-md hover:shadow-emerald-600/30 transition-all flex items-center justify-center gap-2">
